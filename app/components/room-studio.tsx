...
export function RoomStudio() {
  ...
  function cycleSelectedFurnitureFacing() {
    if (!selectedFurniture) return;
    const facings = Object.keys(furnitureFacingLabels) as FurnitureFacing[];
    const nextFacing = facings[(facings.indexOf(selectedFurniture.facing) + 1) % facings.length];
    orientSelectedFurniture(nextFacing);
  }
        ...
        <section className="stage" aria-labelledby="editor-title">
          ...
          <div className="canvas-wrap"><div ref={canvasRef} className={`canvas ${isDraggingFile ? 'is-dragging' : ''} ${pendingFurniture ? 'is-placing-furniture' : ''} ${isPickingCleanup ? 'is-picking-cleanup' : ''}`} id="editor-title" style={room ? { aspectRatio: roomRatio } : undefined} onClick={handleCanvasClick} onDragEnter={() => setIsDraggingFile(true)} onDragLeave={() => setIsDraggingFile(false)} onDragOver={(event) => event.preventDefault()} onDrop={onDrop}>
            {room?.previewUrl ? <div className="editor-media">
              ...
              })}{selectedFurniture && !selectedFurniture.frozen && <div className="canvas-ipad-controls" role="group" aria-label="Comandi sul mobile" style={{ left: `${selectedFurniture.x * 100}%`, top: `${selectedFurniture.y * 100}%` }} onClick={(event) => event.stopPropagation()}><button type="button" aria-label={`Ruota ${selectedFurniture.name} a sinistra`} onClick={() => rotateSelectedFurniture(-5)}>↶</button><button type="button" aria-label={`Sposta ${selectedFurniture.name} in alto`} onClick={() => nudgeSelectedFurniture(0, -.025)}>↑</button><button type="button" aria-label={`Ruota ${selectedFurniture.name} a destra`} onClick={() => rotateSelectedFurniture(5)}>↷</button><button type="button" aria-label={`Sposta ${selectedFurniture.name} a sinistra`} onClick={() => nudgeSelectedFurniture(-.025, 0)}>←</button><button type="button" className="canvas-wall-button" aria-label={`Cambia parete di ${selectedFurniture.name}`} onClick={cycleSelectedFurnitureFacing}>{selectedFurniture.facing === 'front-wall' ? 'Muro ↑' : selectedFurniture.facing === 'left-wall' ? 'Muro ↙' : 'Muro ↘'}</button><button type="button" aria-label={`Sposta ${selectedFurniture.name} a destra`} onClick={() => nudgeSelectedFurniture(.025, 0)}>→</button><button type="button" aria-label={`Rimpicciolisci ${selectedFurniture.name}`} onClick={() => resizeSelectedFurniture(-3)}>−</button><button type="button" aria-label={`Sposta ${selectedFurniture.name} in basso`} onClick={() => nudgeSelectedFurniture(0, .025)}>↓</button><button type="button" aria-label={`Ingrandisci ${selectedFurniture.name}`} onClick={() => resizeSelectedFurniture(3)}>＋</button></div>}</div>}
