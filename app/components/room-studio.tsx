...
const STEP_RENDER = 5;
const RECOGNITION_WAITING_TITLE = 'Riconosco la stanza…';
const RECOGNITION_WAITING_DETAIL = 'Sto cercando muri, pavimento e soffitto. Di solito 30–90 secondi, al massimo circa 2 minuti. Le linee appariranno a fine riconoscimento.';

function recognitionElapsedLabel(seconds: number) {
  return `Passati ${seconds} s…`;
}
...
export function RoomStudio() {
  ...
  const [autoFitElapsedSeconds, setAutoFitElapsedSeconds] = useState(0);
  ...
  const assistantActivity = isVerifyingGeometry
    ...
    : isCreatingFloorplanRoom
      ...
      : isAutoFitting
        ? { working: true, title: RECOGNITION_WAITING_TITLE, detail: `${RECOGNITION_WAITING_DETAIL} ${recognitionElapsedLabel(autoFitElapsedSeconds)}` }
        : isEmptyingRoom
                      ...
                      : isRendering
                        ...
                        : activeStep === STEP_PHOTO
                          ...
                          : activeStep === STEP_PREPARE
                            ? { working: false, title: geometrySaved ? 'Linee salvate' : surfaces.length ? 'Riconoscimento completato' : 'Controllo la foto', detail: geometrySaved ? `Ho salvato: ${recognizedStructure}.` : surfaces.length ? `Ho trovato: ${recognizedStructure}. Scegli se svuotare o usare la foto originale.` : 'Se i bordi sono giusti, puoi continuare con la foto originale.' }
      ...
      <div className="workspace">
        ...
        <section className="stage" aria-labelledby="editor-title">
          ...
          <div className="canvas-wrap">
            ...
            {(isImportingRoom || isCreatingFloorplanRoom || isAutoFitting) && <div className="processing-overlay" role="status"><img className="processing-mascot" src="/materia-assistant.png" alt="Assistente Materia al lavoro" /><strong>{isAutoFitting ? RECOGNITION_WAITING_TITLE : isCreatingFloorplanRoom ? 'Creo la stanza dalla planimetria…' : 'Preparo la foto…'}</strong><small>{isAutoFitting ? RECOGNITION_WAITING_DETAIL : isCreatingFloorplanRoom ? 'Riconosco pareti, porte e finestre. Può richiedere circa un minuto.' : 'Le immagini grandi vengono ottimizzate senza modificare l’originale.'}</small>{isAutoFitting && <small className="processing-elapsed" aria-hidden="true">{recognitionElapsedLabel(autoFitElapsedSeconds)}</small>}</div>}
