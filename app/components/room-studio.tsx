...
export function RoomStudio() {
  ...
  function resizeSelectedFurniture(delta: number) {
    ...
    updateSelectedFurniture({ scale: Math.min(90, Math.max(12, selectedFurniture.scale + delta)), autoScale: false });
  }
    ...
    <main ref={shellRef} className={`app-shell simple-mode step-${activeStep}`}>
      ...
      <div className="workspace">
        ...
        <section className="stage" aria-labelledby="editor-title">
          ...
          <div className="canvas-wrap">{activeStep === 3 && selectedFurniture && <div className="furniture-pencil-toolbar" role="group" aria-label="Comandi Apple Pencil per il mobile"><div className="furniture-pencil-heading"><div><strong>{selectedFurniture.name}</strong><span>Apple Pencil e tocco · comandi sempre visibili</span></div>{selectedFurniture.frozen && <button type="button" className="pencil-unlock" onClick={() => updateSelectedFurniture({ frozen: false })}>◇ SBLOCCA</button>}</div><div className="furniture-pencil-walls" role="group" aria-label="Scegli il muro del mobile">{(Object.keys(furnitureFacingLabels) as FurnitureFacing[]).map((facing) => <button type="button" key={facing} aria-label={`Orienta ${selectedFurniture.name}: ${furnitureFacingLabels[facing]}`} className={selectedFurniture.facing === facing ? 'is-active' : ''} onClick={() => orientSelectedFurniture(facing)} disabled={selectedFurniture.frozen}>{facing === 'front-wall' ? '↑ FRONTE' : facing === 'left-wall' ? '↙ MURO SX' : 'MURO DX ↘'}</button>)}<button type="button" className="straight-button" aria-label={`Raddrizza ${selectedFurniture.name}`} onClick={straightenSelectedFurniture} disabled={selectedFurniture.frozen}>0° DRITTO</button></div><div className="furniture-pencil-actions"><button type="button" aria-label={`Ruota ${selectedFurniture.name} a sinistra`} onClick={() => rotateSelectedFurniture(-5)} disabled={selectedFurniture.frozen}>↶ 5°</button><button type="button" aria-label={`Sposta ${selectedFurniture.name} in alto`} onClick={() => nudgeSelectedFurniture(0, -.04)} disabled={selectedFurniture.frozen}>↑</button><button type="button" aria-label={`Ruota ${selectedFurniture.name} a destra`} onClick={() => rotateSelectedFurniture(5)} disabled={selectedFurniture.frozen}>5° ↷</button><button type="button" aria-label={`Sposta ${selectedFurniture.name} a sinistra`} onClick={() => nudgeSelectedFurniture(-.04, 0)} disabled={selectedFurniture.frozen}>←</button><button type="button" aria-label={`Sposta ${selectedFurniture.name} a destra`} onClick={() => nudgeSelectedFurniture(.04, 0)} disabled={selectedFurniture.frozen}>→</button><button type="button" className="pencil-size-button" aria-label={`Rimpicciolisci ${selectedFurniture.name}`} onClick={() => resizeSelectedFurniture(-3)} disabled={selectedFurniture.frozen}>− <small>PICCOLO</small></button><button type="button" aria-label={`Sposta ${selectedFurniture.name} in basso`} onClick={() => nudgeSelectedFurniture(0, .04)} disabled={selectedFurniture.frozen}>↓</button><button type="button" className="pencil-size-button" aria-label={`Ingrandisci ${selectedFurniture.name}`} onClick={() => resizeSelectedFurniture(3)} disabled={selectedFurniture.frozen}>＋ <small>GRANDE</small></button></div></div>}<div ref={canvasRef} className={`canvas ${isDraggingFile ? 'is-dragging' : ''} ${pendingFurniture ? 'is-placing-furniture' : ''} ${isPickingCleanup ? 'is-picking-cleanup' : ''}`} id="editor-title" style={room ? { aspectRatio: roomRatio } : undefined} onClick={handleCanvasClick} onDragEnter={() => setIsDraggingFile(true)} onDragLeave={() => setIsDraggingFile(false)} onDragOver={(event) => event.preventDefault()} onDrop={onDrop}>
