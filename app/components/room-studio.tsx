...
export function RoomStudio() {
  ...
  function importRoom(file?: File) {
    ...
    setSurfaces([]); setSelectedId(null); setDraft([]); setDrawKind(null); setError(null);
  ...
  function removeRoom() {
    ...
    setRoom(null); setSurfaces([]); setSelectedId(null); setDraft([]); setDrawKind(null); setNotice(null);
  }

  function startDrawing(kind: SurfaceKind = 'wall') {
    if (!room) return;
    setDrawKind(kind); setDraft([]); setSelectedId(null);
  ...
  function finishSurface() {
    ...
    setSurfaces((current) => [...current, surface]); setSelectedId(id); setDraft([]); setDrawKind(null); setError(null);
  ...
  function endVertexDrag(event: ReactPointerEvent<SVGSVGElement>) {
    if (dragVertex && event.pointerId === dragVertex.pointerId) setDragVertex(null);
  }

  function toggleFreeze() {
    if (!selected) return;
    setSurfaces((current) => current.map((surface) => surface.id === selected.id ? { ...surface, frozen: !surface.frozen } : surface));
  ...
  function deleteSelected() {
    ...
    setSurfaces((current) => current.filter((surface) => surface.id !== selected.id)); setSelectedId(null);
  ...
  function applyMaterial() {
    ...
    setSurfaces((current) => current.map((surface) => surface.id === selected.id ? { ...surface, materialId: material.id } : surface));
  ...
  function seedGuidedSurfaces() {
    ...
    setSurfaces(created); setSelectedId(created[0].id);
  ...
  function loadDemoRoom() {
    ...
    setRoomRatio(16 / 9); setSurfaces(created); setSelectedId(created[0].id); setError(null);
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        ...
        <aside className="properties-panel" aria-label="Proprietà">
          ...
          {selected ? <><div className="property-section"><div className="property-title"><span>Protezione superficie</span><span className={`editable-badge ${selected.frozen ? 'frozen' : ''}`}>{selected.frozen ? 'Frozen' : 'Modificabile'}</span></div><button className={`freeze-button ${selected.frozen ? 'is-active' : ''}`} type="button" aria-label={selected.frozen ? 'Sblocca superficie' : 'Freeze superficie'} onClick={toggleFreeze}><span>{selected.frozen ? '◆' : '◇'}</span>{selected.frozen ? 'Sblocca superficie' : 'Freeze superficie'}<small>{selected.frozen ? 'Protetta' : 'Attivo subito'}</small></button></div>
