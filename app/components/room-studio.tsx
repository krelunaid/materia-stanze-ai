        ...
        <section className="stage" aria-labelledby="editor-title">
          ...
          <div className="canvas-wrap"><div ref={canvasRef} className={`canvas ${isDraggingFile ? 'is-dragging' : ''} ${pendingFurniture ? 'is-placing-furniture' : ''} ${isPickingCleanup ? 'is-picking-cleanup' : ''}`} id="editor-title" style={room ? { aspectRatio: roomRatio } : undefined} onClick={handleCanvasClick} onDragEnter={() => setIsDraggingFile(true)} onDragLeave={() => setIsDraggingFile(false)} onDragOver={(event) => event.preventDefault()} onDrop={onDrop}>
            {room?.previewUrl ? <div className="editor-media">
              ...
              })}{selectedFurniture && !selectedFurniture.frozen && <div className="canvas-ipad-controls" role="group" aria-label="Comandi attorno al mobile" style={{ left: `${selectedFurniture.x * 100}%`, top: `${selectedFurniture.y * 100}%`, width: `min(${Math.max(26, selectedFurniture.scale)}%, calc(100% - 112px))` }} onClick={(event) => event.stopPropagation()}><button type="button" className="control-rotate-left" aria-label={`Ruota ${selectedFurniture.name} a sinistra`} onClick={() => rotateSelectedFurniture(-5)}>↶</button><button type="button" className="control-move-up" aria-label={`Sposta ${selectedFurniture.name} in alto`} onClick={() => nudgeSelectedFurniture(0, -.025)}>↑</button><button type="button" className="control-rotate-right" aria-label={`Ruota ${selectedFurniture.name} a destra`} onClick={() => rotateSelectedFurniture(5)}>↷</button><button type="button" className="control-move-left" aria-label={`Sposta ${selectedFurniture.name} a sinistra`} onClick={() => nudgeSelectedFurniture(-.025, 0)}>←</button><button type="button" className="control-move-right" aria-label={`Sposta ${selectedFurniture.name} a destra`} onClick={() => nudgeSelectedFurniture(.025, 0)}>→</button><button type="button" className="control-size-down" aria-label={`Rimpicciolisci ${selectedFurniture.name}`} onClick={() => resizeSelectedFurniture(-3)}>−</button><button type="button" className="control-move-down" aria-label={`Sposta ${selectedFurniture.name} in basso`} onClick={() => nudgeSelectedFurniture(0, .025)}>↓</button><button type="button" className="control-size-up" aria-label={`Ingrandisci ${selectedFurniture.name}`} onClick={() => resizeSelectedFurniture(3)}>＋</button></div>}</div>}
