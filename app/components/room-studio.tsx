...
export function RoomStudio() {
  ...
  return (
    <main ref={shellRef} className={`app-shell simple-mode step-${activeStep} ${(activeStep === 2 && (drawKind || selected)) || (activeStep === 3 && isCorrectingEdges && selected) ? 'has-mobile-surface-actions' : ''}`}>
      ...
      <aside className={`materia-assistant ${assistantActivity.working ? 'is-working' : ''}`} aria-live="polite" aria-label="Assistente Materia"><div className="materia-assistant-bubble"><strong>{assistantActivity.title}</strong><span>{assistantActivity.detail}</span></div><div className="materia-assistant-character" aria-hidden="true"><img src="/materia-assistant.png" alt="" /><span className="materia-assistant-pupil is-left" /><span className="materia-assistant-pupil is-right" /></div></aside>
