      ...
      <div className="workspace">
        ...
        <aside className="properties-panel" aria-label="Proprietà">
          ...
          {selected ? <><div className="property-section"><div className="property-title"><span>Nome superficie</span><span className="editable-badge">Personalizzabile</span></div><div className="rename-control"><input aria-label="Nome superficie" value={renameDraft} onChange={(event) => setRenameDraft(event.target.value)} /><button type="button" onClick={renameSelected} disabled={!renameDraft.trim() || renameDraft.trim() === selected.name}>Salva</button></div></div><div className="property-section"><div className="property-title"><span>Protezione superficie</span><span className={`editable-badge ${selected.frozen ? 'frozen' : ''}`}>{selected.frozen ? 'Frozen' : 'Modificabile'}</span></div><button className={`freeze-button ${selected.frozen ? 'is-active' : ''}`} type="button" aria-label={selected.frozen ? 'Sblocca superficie' : 'Freeze superficie'} onClick={toggleFreeze}><span>{selected.frozen ? '◆' : '◇'}</span>{selected.frozen ? 'Sblocca superficie' : 'Freeze superficie'}<small>{selected.frozen ? 'Protetta' : 'Attivo subito'}</small></button><button className="freeze-others-button" type="button" onClick={freezeAllExceptSelected}>Blocca tutto tranne {selected.name}</button></div>
            ...
            <div className="property-section furniture-section"><div className="property-title"><span>Mobili nella stanza</span><span className="editable-badge">{placedFurniture.length + customRequests.length} scelti</span></div><button className="upload-furniture-button" type="button" onClick={() => furnitureInputRef.current?.click()}>＋ Carica la foto di un mobile</button>{placedFurniture.length || customRequests.length ? <div className="selected-assets">{placedFurniture.map((item, index) => <button type="button" className={selectedFurnitureId === item.id ? 'is-selected' : ''} key={item.id} onClick={() => setSelectedFurnitureId(item.id)}>{item.name} {placedFurniture.filter((candidate) => candidate.name === item.name).length > 1 ? index + 1 : ''}<span>{item.frozen ? '◆' : '›'}</span></button>)}{customRequests.map((item) => <button type="button" key={item} onClick={() => setCustomRequests((current) => current.filter((name) => name !== item))}>{item}<span>×</span></button>)}</div> : <p className="no-results">Cerca un mobile, toccalo e poi indica direttamente il punto sul pavimento.</p>}{selectedFurniture && <div className="furniture-controls"><div><strong>{selectedFurniture.name}</strong><span>{selectedFurniture.frozen ? 'Posizione bloccata' : 'Usa i tastini: sono pensati per iPad e funzionano anche senza trascinare.'}</span></div><div className="furniture-facing-controls" aria-label="Parete di orientamento"><span>Schienale verso</span>{(Object.keys(furnitureFacingLabels) as FurnitureFacing[]).map((facing) => <button type="button" key={facing} className={selectedFurniture.facing === facing ? 'is-active' : ''} onClick={() => orientSelectedFurniture(facing)} disabled={selectedFurniture.frozen}>{furnitureFacingLabels[facing]}</button>)}</div><div className="furniture-touch-title">Sposta</div><div className="furniture-nudge-grid" role="group" aria-label="Sposta il mobile con i pulsanti"><button type="button" onClick={() => nudgeSelectedFurniture(0, -.025)} disabled={selectedFurniture.frozen} aria-label="Sposta mobile in alto">↑ Su</button><button type="button" onClick={() => nudgeSelectedFurniture(-.025, 0)} disabled={selectedFurniture.frozen} aria-label="Sposta mobile a sinistra">← Sinistra</button><button type="button" onClick={() => nudgeSelectedFurniture(.025, 0)} disabled={selectedFurniture.frozen} aria-label="Sposta mobile a destra">Destra →</button><button type="button" onClick={() => nudgeSelectedFurniture(0, .025)} disabled={selectedFurniture.frozen} aria-label="Sposta mobile in basso">↓ Giù</button></div><div className="furniture-touch-title">Inclina</div><div className="furniture-rotation-grid" role="group" aria-label="Ruota il mobile con i pulsanti"><button type="button" onClick={() => rotateSelectedFurniture(-5)} disabled={selectedFurniture.frozen || selectedFurniture.rotation <= -35} aria-label="Ruota mobile a sinistra">↶ Sinistra</button><button type="button" onClick={() => updateSelectedFurniture({ rotation: 0 })} disabled={selectedFurniture.frozen || selectedFurniture.rotation === 0} aria-label="Raddrizza mobile">{Math.round(selectedFurniture.rotation)}°</button><button type="button" onClick={() => rotateSelectedFurniture(5)} disabled={selectedFurniture.frozen || selectedFurniture.rotation >= 35} aria-label="Ruota mobile a destra">Destra ↷</button></div><div className="furniture-touch-title">Dimensione</div><div className="furniture-control-grid"><button type="button" onClick={() => resizeSelectedFurniture(-3)} disabled={selectedFurniture.frozen} aria-label="Rimpicciolisci mobile">− Piccolo</button><button type="button" onClick={() => resizeSelectedFurniture(3)} disabled={selectedFurniture.frozen} aria-label="Ingrandisci mobile">＋ Grande</button></div><button className={`auto-size-furniture-button ${selectedFurniture.autoScale ? 'is-active' : ''}`} type="button" onClick={restoreAutomaticFurnitureScale} disabled={selectedFurniture.frozen || selectedFurniture.autoScale}>{selectedFurniture.autoScale ? `✓ Auto attivo · ${Math.round(selectedFurniture.scale)}%` : '◎ Ripristina misura automatica'}</button><button className={`freeze-furniture-button ${selectedFurniture.frozen ? 'is-active' : ''}`} type="button" onClick={() => updateSelectedFurniture({ frozen: !selectedFurniture.frozen })}>{selectedFurniture.frozen ? '◇ Sblocca posizione' : '◆ Blocca posizione'}</button><button className="remove-furniture-button" type="button" onClick={removeSelectedFurniture} disabled={selectedFurniture.frozen}>Rimuovi mobile</button></div>}<p className="material-search-note">I tastini permettono regolazioni precise su iPad. Grok usa posizione, parete, angolo e dimensione nel render finale.</p></div>
